...
num_epochs= 55
batch_size=128
log_no='deepIso_pointNet_isoDetect_c_relu_v5_batchNorm' 
#model_load=
activation_func=2
val_start=0
...
# T-Net
# ------------------------------------------------------------------------------------------------------------------------------------------------------------
T_net_W_conv0 = weight_variable([1, 3 , 1, 8], 'W_conv0')# 64 kernels each having [1,3] sized filter.
T_net_b_conv0 = bias_variable([8], 'b_conv0') #for each of feature maps

T_net_W_conv1 = weight_variable([1, 1 , 8, 16], 'W_conv1')# #20x193
T_net_b_conv1 = bias_variable([16], 'b_conv1') #for each of feature maps

T_net_W_conv2 = weight_variable([1, 1, 16, 32],  'W_conv2')  #18x186
T_net_b_conv2 = bias_variable([32], 'b_conv2') 
...
nw_W_conv0 = weight_variable([1, 3 , 1, 8], 'nw_W_conv0')# 64 kernels each having [1,3] sized filter.
nw_b_conv0 = bias_variable([8], 'nw_b_conv0') #for each of feature maps

nw_W_conv01 = weight_variable([1, 1 , 8, 16], 'nw_W_conv0')# 64 kernels each having [1,3] sized filter.
nw_b_conv01 = bias_variable([16], 'nw_b_conv0') #for each of feature maps

#--------------------

F_net_W_conv0 = weight_variable([1, 16 , 1, 8], 'F_W_conv0')# 64 kernels each having [1,3] sized filter.
F_net_b_conv0 = bias_variable([8], 'F_b_conv0') #for each of feature maps

F_net_W_conv1 = weight_variable([1, 1 , 8, 16], 'F_W_conv1')# #20x193
F_net_b_conv1 = bias_variable([16], 'F_b_conv1') #for each of feature maps

F_net_W_conv2 = weight_variable([1, 1, 16, 32],  'F_W_conv2')  #18x186
F_net_b_conv2 = bias_variable([32], 'F_b_conv2') 
...
#--------------------------
nw_W_conv1 = weight_variable([1, 16 , 1, 8], 'nw_W_conv1')# 64 kernels each having [1,3] sized filter.
nw_b_conv1 = bias_variable([8], 'nw_b_conv1') #for each of feature maps

nw_W_conv2 = weight_variable([1, 1 , 8, 16], 'nw_W_conv2')# #20x193
nw_b_conv2 = bias_variable([16], 'nw_b_conv2') #for each of feature maps

nw_W_conv3 = weight_variable([1, 1, 16, 32],  'nw_W_conv3')  #18x186
nw_b_conv3 = bias_variable([32], 'nw_b_conv3')  # global feature
...
if (activation_func==1):       
    ...
    T_net_mlp_2 =  tf.nn.relu(conv2d(tf.layers.batch_normalization(T_net_mlp_1, training=is_train), T_net_W_conv2) + T_net_b_conv2) # now the layer is : b x n x 1024
    T_net_maxpool_points=tf.nn.max_pool(tf.layers.batch_normalization(T_net_mlp_2,training=is_train), ksize=[1, datapoints, 1, 1], strides=[1, datapoints, 1, 1], padding='SAME')
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    nw_mlp_0 = tf.nn.relu(conv2d(tf.reshape(T_net_point_transformation[:, :, :], [-1, datapoints, 3, 1]), nw_W_conv0) + nw_b_conv0) # now t$
    nw_mlp_01 = tf.nn.relu(conv2d(tf.layers.batch_normalization(nw_mlp_0, training=is_train), nw_W_conv01) + nw_b_conv01) # now the layer i$
    nw_mlp_01 = tf.reshape(tf.layers.batch_normalization(nw_mlp_01,training=is_train),[-1,datapoints,16])



    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    F_net_mlp_0 = tf.nn.relu(conv2d(tf.reshape(nw_mlp_01[:, :, :], [-1, datapoints, 16, 1]), F_net_W_conv0) + F_net_b_conv0) # now the layer is : b x n x 64  
    ...
    F_net_mlp_2 =  tf.nn.relu(conv2d(tf.layers.batch_normalization(F_net_mlp_1, training=is_train), F_net_W_conv2) + F_net_b_conv2) # now the layer is : b x n x 1024
    F_net_maxpool_points=tf.nn.max_pool(tf.layers.batch_normalization(F_net_mlp_2,training=is_train), ksize=[1, datapoints, 1, 1], strides=[1, datapoints, 1, 1], padding='SAME')
    ...
    F_net_point_transformation = tf.matmul(nw_mlp_01, F_net_point_transformation_matrix) # dimension [n x 64]
    ...
    nw_mlp_3 = tf.nn.relu(conv2d(tf.layers.batch_normalization(nw_mlp_2, training=is_train), nw_W_conv3) + nw_b_conv3) # now the layer is : b x n x 1024
    nw_maxpool_points=tf.nn.max_pool(tf.layers.batch_normalization(nw_mlp_3,training=is_train), ksize=[1, datapoints, 1, 1], strides=[1, datapoints, 1, 1], padding='SAME')
...
#################dataset##################################################################
#f=open(path+'feature_list/'+'pointCloud_training_set', 'rb')
...
#
#x=np.zeros((1, len(train_data[0])))
#y=np.zeros((1, len(train_data[0])))
#z=np.zeros((1, len(train_data[0])))
#for i in range (0, len(train_data[0])):
#    x[0, i]=train_data[0][i][1]
#    y[0, i]=train_data[0][i][0]
#    z[0, i]=train_data[0][i][2]
#
...
#        if epoch_idx%5==0:
#            set_learn_rate=round(set_learn_rate/2.0, 6)
